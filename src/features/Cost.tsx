...
export function Cost({ data, pricing, range, onRange, onRefresh, loading }: Props) {
  ...
  return <main className="page cost-page">
    ...
    <section className="cost-overview">
      ...
      <div className="panel cost-session-summary"><div className="panel-title"><div><h2>{t("cost.currentModel")}</h2><p>{selectedModel || t("cost.noSession")}</p></div><Database size={19} /></div><strong className="cost-amount">{cost ? usd(cost.totalCost) : t("pricing.unavailable")}</strong><span className="cost-caption">{t("cost.currentSessionEstimate")}</span><div className="cost-meta"><span>{formatTokens(totals.totalTokens, language)} {t("dashboard.totalTokens")}</span><span>{selectedModel || t("cost.noSession")}</span></div></div>
    </section>
